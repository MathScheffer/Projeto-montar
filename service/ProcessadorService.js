...
exports.criar = async(reqBody,callback) => {
    ...
    processadorRepository.criar(
        ...
        reqBody.tdp,
        consumo_max,
        reqBody.quantidade,(err,processador) => {
...
exports.atualizarQuantidade = async(qtd,id) => {
    ...
    const processadorInicial = await fetchProcessador({"id":id},["quantidade"])
    const procAtual = await processadorRepository.atualizar(params,id)
    const processadorFinal = await fetchProcessador({"id":id},["quantidade"])
...
exports.fetchProcessador = async(params,specificationOfReturn) => {
    if(specificationOfReturn){
        ...
        return JSON.stringify(processador);
    }else{
        let paramsKeys = []

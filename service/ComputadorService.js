...
const processadorService = new ProcessadorService()

module.exports = class ComputadorService{
    constructor(){
        this.integradorHardwareFaltante = new Map();
    }
    criar = (reqBody,callback) => {
    ...
    adicionarProcessador = async(id,callback) => {
        try{
            let finalMessage = {};
            ...
            if(processador && processador.quantidade > 0){            
                ...
                if(placasMae){
                    //1
                    const quantidadeAtual = (await processadorService.fetchProcessador({"id":id},["quantidade"])).quantidade;
                    //2
    ...
    adicionarPlacaMae = async(id,callback) => {
        try{
            let finalMessage = {};
    ...
    adicionarRam = async(id,callback) => {
        try{
            let finalMessage={};
    ...
    adicionarArmazenamento = async(id,callback) => {
        try{
            let finalMessage = {};
    ...
    adicionarVga = async(id,callback) => {
        try{
            let finalMessage = {};
    ...
    adicionarFonte = async(id,callback) => {
        try{
            let finalMessage = {};
    ...
    adicionarComputador = async(userId,callback) => {
        ...
        computadorRepository.criar(userId,
            ...
            if(err){
                ...
                (async() =>{
                    await this.atualizarQuantidadeHardware("processador",processador.id);
                })()
                
    
                if(this.integradorHardwareFaltante.size > 0) {
                    const error = {
                        status:422,
                        message:"Ha hardwares faltando no estoque!",
                        hardwares:Computador.integradorToJson(this.integradorHardwareFaltante)
                    }
                    callback(error,null)
                }else{
                    callback(null,{
    ...
    async atualizarQuantidadeHardware(nomeHardware,id){
        if(nomeHardware === "processador"){
            const quantidadeDesejada = Computador.integradorRequisicaoQuantidades.get("Processador");
            let qtdAtual = (await processadorService.fetchProcessador({"id":id},["quantidade"])).quantidade;
            ...
            if(qtdAtual > 0){
                ...
                await processadorService.atualizarQuantidade((qtdAtual),id);
                this.integradorHardwareFaltante.set(nomeHardware,
                    {"quantidade":0}); 
            }else{
                this.integradorHardwareFaltante.set(nomeHardware,
                    {"quantidade":qtdAtual}); 
...
exports.adicionarComputador = async(userId,callback) => {
    //Computador.integradorHardwareFaltante.clear();
    ...
    computadorRepository.criar(userId,
        ...
        if(err){
            ...
            (async() =>{
                await atualizarQuantidadeHardware("processador",processador.id);
            })()
